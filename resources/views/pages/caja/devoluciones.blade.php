...
<div class="p-4 mx-auto max-w-screen-2xl md:p-6">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-neutral-800 dark:bg-neutral-900 overflow-hidden">
        ...
        <div class="p-4 border-t border-gray-200 dark:border-neutral-800">{{ $devoluciones->links() }}</div>
        @endif
    </div>
</div>
...
<div id="modalDevolucion" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
    <div class="w-full max-w-lg rounded-2xl bg-white dark:bg-neutral-900 shadow-2xl" @click.stop>
        <div class="flex items-center justify-between p-5 border-b border-gray-200 dark:border-neutral-800">
            ...
            <button onclick="document.getElementById('modalDevolucion').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
        ...
        <form action="{{ route('caja.devoluciones.store') }}" method="POST" class="p-5 space-y-4">
            ...
            <div>
                ...
                <select name="venta_id" id="ventaSelect" onchange="cargarProductosVenta()" required
                        class="h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm focus:border-brand-500 dark:border-neutral-700 dark:bg-neutral-900 dark:text-white">
            ...
            <div>
                ...
                <textarea name="motivo" rows="3" required
                          class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:border-brand-500 dark:border-neutral-700 dark:bg-neutral-900 dark:text-white"
            ...
            <div class="flex gap-3 justify-end pt-2">
                <button type="button" onclick="document.getElementById('modalDevolucion').classList.add('hidden')"
                        class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 dark:border-neutral-700 dark:text-gray-400">Cancelar</button>
